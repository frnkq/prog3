...
class DaoMysql
{
    ...
    {
      $query = MyPDO::GetPDO()->ReturnQuery("select * from ".self::$tableName." where legajo=:identifier");
      $query->bindValue(":identifier", $identifier, PDO::PARAM_STR);
      $query->execute();
      $object = null;
      try
      {
        $object = $query->fetchObject(self::$entity);
      }
      catch(Exception $e)
      {
    ...
    public static function DeleteByIdentifier($identifier)
    {
        if(self::GetByIdentifier($identifier) == null)
          return false;
        $deleteQuery = "delete from ".self::$tableName." where legajo=:legajo";//CHANGE
        $query = MyPDO::GetPDO()->ReturnQuery($deleteQuery);
        $query->bindValue(':legajo', $identifier, PDO::PARAM_STR);
